...
public class WorkService {
	...
	static ProcessStep processStepResource = null;
	public WorkService() {  }
	
	public void setWorkStatemachine(ProcessStep[] processSteps) {
		sendUpdate = true;
		processStepResource = processSteps[0];
	...
	static class HttpRequestHandler implements HttpAsyncRequestHandler<HttpRequest> {
		...
		private void handleInternal(final HttpRequest request, final HttpResponse response, final HttpContext context)
			...
			if(sendUpdate) {
				...
				if(processStepResource != null) {
					body = new NStringEntity(new Gson().toJson(processStepResource), ContentType.APPLICATION_JSON);
				}else {
