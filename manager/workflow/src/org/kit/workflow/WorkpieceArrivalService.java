...
/*
 * WorkpieceArrivalService
 * Description: This service provides and interface for notification of work piece arrival.
 * 
*/
public class WorkpieceArrivalService {

	static ProductionorderService productionorderService;
	public WorkpieceArrivalService(ProductionorderService service) { 
		injectProductionorderConsumer(service);
	}
	
	public void injectProductionorderConsumer(ProductionorderService service) {
		WorkpieceArrivalService.productionorderService = service;
	}
	...
	static class HttpRequestHandler implements HttpAsyncRequestHandler<HttpRequest> {
		...
		private void handleInternal(final HttpRequest request, final HttpResponse response, final HttpContext context)
			...
			final String chassiId = EntityUtils.toString(((HttpEntityEnclosingRequest)request).getEntity());
			System.out.println("request.toString(): " + chassiId );
			
			productionorderService.getProductionOrder(chassiId);
