...
public class WorkflowExecutor {
	...
	public boolean putInstructionList() {
		...
		data.add(new PresentationData(
				"{\"id\": \"nutrunner\",\"status\": \"incomplete\",\"text\": \""+processStep.getBillOfOperation().iterator().next().getInstructionTexts().iterator().next().getText()+"\",\"count\": \"" + nutrunnerbatch + "\",\"image\": \"na\",\"progress\": \"0/"+nutrunnerbatch+"\"}"));
//		data.add(new PresentationData(
...
//				"{\"id\": \"completed\",\"status\": \"incomplete\",\"text\": \"Hit the 'F' key when finished\",\"count\": \"1\",\"image\": \"na\",\"progress\": \"0/1\"}"));
	...
	public boolean postJobToNutrunner() {
		...
		String srurl = "http://192.168.0.33:8085/job";
	...
	public boolean postInstructionUpdate() {
		...
		if (nutrunnercounter >= nutrunnerbatch) {
			data.add(new PresentationData( "{\"id\": \"nutrunner\",\"status\": \"complete\",\"text\": \""+processStep.getBillOfOperation().iterator().next().getInstructionTexts().iterator().next().getText()+"\",\"count\": \""+nutrunnerbatch+"\",\"image\": \"na\",\"progress\": \"" + nutrunnercounter + "/"+nutrunnerbatch+"\"}"));

		} else {
			data.add(new PresentationData( "{\"id\": \"nutrunner\",\"status\": \"incomplete\",\"text\": \""+processStep.getBillOfOperation().iterator().next().getInstructionTexts().iterator().next().getText()+"\",\"count\": \""+nutrunnerbatch+"\",\"image\": \"na\",\"progress\": \"" + nutrunnercounter + "/"+nutrunnerbatch+"\"}"));
		}
	...
	public boolean getLastJobFromNutrunner() {
		...
		String srurl = "http://192.168.0.33:8085/lastjob";
		...
		try {
			...
			if(previous != current || nutrunnercounter>=nutrunnerbatch)  {
				previous = current;
				//nutrunnercounter++;
