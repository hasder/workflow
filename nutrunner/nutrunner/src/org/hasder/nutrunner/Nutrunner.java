...
public class Nutrunner {
	...
	public static void main(String argv[]) throws Exception {
		...
		String nutrunnerport = config.getProperty("nutrunnerport");
		String ownip = config.getProperty("ownip");
		String serviceregistryip = config.getProperty("serviceregistryip");
		if (nutrunnerip == null  ) {
			System.err.println("Configuration file error with nutrunnerip. Please check config.cfg in the root of the java jar executable.");
		}
		if (nutrunnerport == null ) {
			System.err.println("Configuration file error with nutrunnerport. Please check config.cfg in the root of the java jar executable.");
		}
		if ( ownip == null ) {
			System.err.println("Configuration file error with ownip. Please check config.cfg in the root of the java jar executable.");
		}
		if (serviceregistryip == null ) {
			System.err.println("Configuration file error with serviceregistryip. Please check config.cfg in the root of the java jar executable.");
		}
//		if (nutrunnerip == null || nutrunnerport == null || ownip == null || serviceregistryip == null ) {
//			System.err.println("Configuration file error. Please check config.cfg in the root of the java jar executable.");
//		}
		
		Nutrunner nutrunner = new Nutrunner();
		nutrunner.startServiceProvider();
		
		ServiceRegistration registration = new ServiceRegistration(ownip, serviceregistryip);
		registration.doPeriodicRegistration();

		//nutrunner.clientSocket = new Socket("192.168.0.35", 4545);
		nutrunner.clientSocket = new Socket(nutrunnerip, Integer.parseInt(nutrunnerport));
